...
define(['app'], function (app) {
	...
	.controller("regController",["$scope","$http","user","$timeout","$state","zone","modalService",function($scope,$http,user,$timeout,$state,zone,modalService){
		$scope.user = {
			isStaff:null,
			username:"",
			password:"",
		...
		$scope.$watch("user.username",function(n,o){
			if(n!=o&&n){
					...
					$http({
						method:"POST",
						url:"/activity/wx/dragon/api/register/checkName.jsp",
						data:"name="+n
		...
		$scope.save = function(){
			...
			if(!$scope.psCheck){
				$scope.alert("两次输入密码不一样");
				return;
			}
			var loadModal = modalService.genLoadModal();
			$http({
				method:"POST",
				url:"/activity/wx/dragon/api/register/save.jsp",
				...
				if(data.state==200){
					...
					$scope.alert("注册成功")
					.result.then(function(){$state.go("enter");});
				}
				else if(data.state == 201){
					$scope.alert("该手机号码已注册");
				}
